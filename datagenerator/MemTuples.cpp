...
int CMemTuples::GetTuples(LPTuple tuples, int nMaxTuples, int& nTuplesRet)
{
	...
	//cout << "Current:" << m_nCurrentTuple << " First:" <<
	//		m_nFirstTuple << "Last: "<< m_nLastTuple << " nRet: "<<nTuplesRet << endl;
	...
	else
	{
		...
		else
		{
			 ...
			 {
				memcpy(&tuples[nFirstRead], &m_lpTuples[m_nFirstTuple],
						(size_t)(sizeof(Tuple)*(nTuplesRet - nFirstRead)));
