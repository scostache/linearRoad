...
public class TollSegmentState {
	...
	public void markAndClearAccidents(Tuple2<Boolean, Long> value) {
		if (value._1() && value._2() > accidentInfo._1()) {
			...
			this.accidentInfo = new Tuple2(accidentInfo._1(), value._2()); // remember
																			// time
																			// at
																			// which
																			// is
	...
	public boolean needToOutputAccident(long time, int lane) {
		...
		if (lane != 4) {
			...
			if (minute_vid >= minute_acc &&  time > accidentInfo._1 && 
				minute_vid < minute_clear && time < accidentInfo._2) {
				res = true;
			}
		}
		return res;
